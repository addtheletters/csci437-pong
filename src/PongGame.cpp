...
const std::string PongGame::FONT_FILENAME =
...
void PongGame::init() {
  ...
  window_ = std::unique_ptr<sf::RenderWindow>(new sf::RenderWindow(
      sf::VideoMode(800, 600, 32), "Ben Zhang's pong - SFML"));

  // create AI controller
  ai_ = std::unique_ptr<AutoPlayer>(new AutoPlayer(-1.0f, 2.0f));
  ai_one_ = std::unique_ptr<AutoPlayer>(new AutoPlayer(1.0f, 2.0f));

  // load display font
  if (!font_.loadFromFile(FONT_FILENAME)) {
    std::cout << "Is " << FONT_FILENAME << " in the correct place?"
              << std::endl;
  }

  // load ball sound
  if (!ball_sound_.loadFromFile(BALL_SOUND_FILENAME)) {
    std::cout << "Is " << BALL_SOUND_FILENAME << " in the correct place?"
              << std::endl;
  }
  // load miss sound
  if (!miss_sound_.loadFromFile(MISS_SOUND_FILENAME)) {
    std::cout << "Is " << MISS_SOUND_FILENAME << " in the correct place?"
              << std::endl;
  }
  ...
  while (window_->isOpen()) {
    ...
    while (window_->pollEvent(evnt)) {
      ...
      // Window resized
        ...
        std::cout << "Window size set to " << evnt.size.width << "x"
                  << evnt.size.height << std::endl;
        window_->setView(
            sf::View(sf::FloatRect(0, 0, evnt.size.width, evnt.size.height)));
        reset();
        continue;
      }
      ...
      // Window lost focus / minimized
        ...
        // U toggles AI control of the left paddle ("player 1")
        else if (evnt.key.code == sf::Keyboard::U) {
          p1_ai_ = !p1_ai_;
        }
    ...
    if (scorer_ != 0) {
      ...
      std::cout << "Score: p1-" << p1_score_ << " ; p2-" << p2_score_
                << std::endl;
    ...
    if (p1_ai_) {
      ...
      AutoPlayer::Action ai_action = ai_one_->getAIAction(
          window_->getSize(), entities_["ball"]->getPos(),
          entities_["ball"]->getVel(), entities_["p1"]->getPos());
    ...
    if (p2_ai_) {
      // move AI-controlled paddle
      AutoPlayer::Action ai_action = ai_->getAIAction(
          window_->getSize(), entities_["ball"]->getPos(),
          entities_["ball"]->getVel(), entities_["p2"]->getPos());
      if (ai_action == AutoPlayer::Action::MOVE_DOWN)
...
void PongGame::drawText(sf::Text text, int font_size, sf::Vector2f position,
...
void PongGame::resetEntities() {
  ...
  float ball_start_angle =
      BALL_START_ANGLE_RANGE * (((std::rand() % 1000) / 1000.0) - 0.5);
  if ((!p2_ai_ || (p2_ai_ && p1_ai_)) && std::rand() % 2) {
    ball_start_angle += 3.14159;
  }

  entities_["ball"]->setVel(
      vecutil::fromPolar(BALL_START_SPEED, ball_start_angle));
  ...
  entities_["p2"] = std::unique_ptr<Entity>(new Paddle(
      sf::Vector2f(PADDLE_THICKNESS, PADDLE_HEIGHT),
      sf::Vector2f(window_size.x - PADDLE_BACK_DISTANCE, window_size.y / 2)));

  // add obstacle if desired
  if (obstacle_) {
    entities_["o1"] = std::unique_ptr<Entity>(
        new Paddle(sf::Vector2f(PADDLE_THICKNESS * 2, PADDLE_HEIGHT * 2 / 3),
                   sf::Vector2f(window_size.x / 2, window_size.y * 2 / 3)));
    entities_["o2"] = std::unique_ptr<Entity>(
        new Paddle(sf::Vector2f(PADDLE_THICKNESS * 2, PADDLE_HEIGHT * 2 / 3),
                   sf::Vector2f(window_size.x / 2, window_size.y / 3)));
  }
}
...
void PongGame::drawMenu() {
  ...
  drawText(title_text, TITLE_FONT_SIZE,
           sf::Vector2f(half_window.x, half_window.y - 120), true, true);
  ...
  drawText(tutorial_text, TUTORIAL_FONT_SIZE,
           sf::Vector2f(half_window.x, half_window.y + 150), true, false);
}
...
void PongGame::drawScore() {
  ...
  sf::Text p1_score_text(
      std::to_string(p1_score_) + "/" + std::to_string(POINTS_TO_WIN), font_);
  drawText(p1_score_text, SCORE_FONT_SIZE,
           sf::Vector2f(half_window.x - half_window.x / 2, half_window.y), true,
           true);

  // draw right hand player's score
  sf::Text p2_score_text(
      std::to_string(p2_score_) + "/" + std::to_string(POINTS_TO_WIN), font_);
  ...
  drawText(notif_text, CAPTION_FONT_SIZE,
           sf::Vector2f(half_window.x, half_window.y - 50), true, false);

  // draw bottom-center tip
  sf::Text bottom_tip_text(std::string("press [space] to ") +
                               (paused_ ? "start/unpause" : "pause"),
                           font_);
  drawText(bottom_tip_text, TUTORIAL_FONT_SIZE,
           sf::Vector2f(half_window.x, window_->getSize().y - 50), true, false);

  // draw difficulty in top left
  std::stringstream difficulty_val_stream;
  difficulty_val_stream << std::fixed << std::setprecision(2)
  ...
  drawText(difficulty_text, CORNER_FONT_SIZE, sf::Vector2f(10, 10), false,
           false);
  ...
  sf::Text obstacle_text(std::string("[T] obstacle ") +
                             ...
                             "\n" + "[up]/[down] to move p2 paddle\n" +
                             "[U] p1 AI " + (p1_ai_ ? "enabled" : "disabled\n"),
                         font_);
  drawText(obstacle_text, CORNER_FONT_SIZE,
           sf::Vector2f(window_->getSize().x - 300, 10), false, false);
}
