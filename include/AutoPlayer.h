...
public:
  ...
  // area.
  Action getAIAction(sf::Vector2u window_size, sf::Vector2f ball_pos,
                     sf::Vector2f ball_speed, sf::Vector2f paddle_pos);

private:
  // A positive or negative value whose sign designates the 'away' direction.
  float away_;

  // Allowed vertical distance from the ball within which the AI takes no
  // action.
  float laziness_;
};
